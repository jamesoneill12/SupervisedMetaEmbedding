...
def load_vectors(dataset='simlex'):
    pass

def evaluate_similarity(w, X, y):
    """
    Calculate Spearman correlation between cosine word_similarity of the model
    and human rated word_similarity of word pairs
...
def evaluate_relational_similarity(w, X, y, target_vector, concat_target=False):

    words = list(w['words'].keys())


    if concat_target==False:
        mean_vector = np.mean(w[target_vector].data.numpy(), axis=0, keepdims=True)
        A = np.vstack(w[target_vector][list(w['words'].keys()).index(word)].data.numpy() for word in X[:, 0])
        B = np.vstack(w[target_vector][list(w['words'].keys()).index(word)].data.numpy() for word in X[:, 1])
    else:
        A = np.vstack(np.concatenate((np.squeeze(w[target_vector][list(w['words'].keys()).index(word)].data.numpy()),
                                                np.squeeze(concat_target[target_vector.upper()][word])), axis=0) for word in X[:, 0])
        B = np.vstack(np.concatenate((np.squeeze(w[target_vector][list(w['words'].keys()).index(word)].data.numpy()),
                                      np.squeeze(concat_target[target_vector.upper()][word])), axis=0) for word in X[:, 1])

    scores = np.array([v1.dot(v2.T)/(np.linalg.norm(v1)*np.linalg.norm(v2)) for v1, v2 in zip(A, B)])
    return scipy.stats.spearmanr(scores, y).correlation


# passes relational word vectors
def evaluate_on_all_relational(w, target_vector, concat_target=False):

    # Calculate results on word_similarity
    logger.info("Calculating word_similarity benchmarks")
    similarity_tasks = {
        "MEN": fetch_MEN(),
        "WS353": fetch_WS353(),
        "WS353R": fetch_WS353(which="relatedness"),
        "WS353S": fetch_WS353(which="word_similarity"),
    ...
    for name, data in iteritems(similarity_tasks):
        similarity_results[name] = evaluate_relational_similarity(w, data.X, data.y, target_vector, concat_target)
