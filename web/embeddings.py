...
def fetch_GloVe(dim=300, corpus="wiki-6B", normalize=True, lower=False, clean_words=False):
    ...
    _ = _fetch_file(url=download_file[corpus],
                           data_dir=EMBEDDING_PATH,
                           uncompress=True,
                           verbose=1)

    Path = path.join(EMBEDDING_PATH, embedding_file[corpus][dim])
...
def fetch_HPCA(which='hpca', normalize=True, lower=False, clean_words=False):
    """
...
def fetch_FastText(lang="en", normalize=True, lower=False, clean_words=False):
    ...
    path = _fetch_file(url=url_vec, data_dir=EMBEDDING_PATH,
                       uncompress=False,
                       verbose=1)
