...
ANALOGY_PATH = ROOT_PATH + 'analogy'
...
def fetch_google_analogy():
    ...
    with open(_fetch_file(url, ANALOGY_PATH+"/EN-GOOGLE", verbose=0), "r") as f:
        L = f.read().splitlines()
...
def fetch_msr_analogy():
    ...
    with open(_fetch_file(url, ANALOGY_PATH+"/EN-MSR", verbose=0), "r") as f:
        L = f.read().splitlines()
...
def fetch_semeval_2012_2(which="all", which_scoring="golden"):
    ...
    ----------
    DA Jurgens et al.,
    "Measuring degrees of relational word_similarity. In *SEM 2012: The First Joint Conference on Lexical
    ...
    degrees of relational word_similarity. The challenge is to determine the degrees of relational word_similarity between a
    ...
    # Every question is formed as word_similarity to analogy category that is
