...
"""
 Functions for fetching word_similarity datasets
"""
...
SIMILARITY_PATH = ROOT_PATH+ 'word_similarity'

def fetch_MTurk():
    """
    Fetch MTurk dataset for testing attributional word_similarity
    ...
    data = _get_as_pd('https://www.dropbox.com/s/f1v4ve495mmd9pw/EN-TRUK.txt?dl=1',
                      SIMILARITY_PATH, header=None, sep=" ").values
...
def fetch_MEN(which="all", form="natural"):
    """
    Fetch MEN dataset for testing word_similarity and relatedness
...
def fetch_WS353(which="all"):
    ...
    relatedness word_similarity

    Parameters
    ----------
    which : 'all': for both relatedness and attributional word_similarity,
            'relatedness': for relatedness word_similarity
            'word_similarity': for attributional word_similarity
...
def fetch_RW():
    """
    Fetch Rare Words dataset for testing attributional word_similarity
    ...
    Notes
    -----
    2034 word pairs that are relatively rare with human word_similarity scores. Rare word selection: our choices of
...
def fetch_multilingual_SimLex999(which="EN"):
    """
    Fetch Multilingual SimLex999 dataset for testing attributional word_similarity
    ...
    Authors description:
    Multilingual SimLex999 resource consists of translations of the SimLex999 word word_similarity data set to
    ...
    (the language in which word pairs are presented to the human judges) on the resulted word_similarity scores
    ...
    if which == "EN":
        data = _get_as_pd('https://www.dropbox.com/s/nczc4ao6koqq7qm/EN-MSIM999.txt?dl=1',
                          SIMILARITY_PATH, header=None, encoding='utf-8', sep=" ")
    elif which == "DE":
...
def fetch_SimLex999():
    """
    Fetch SimLex999 dataset for testing attributional word_similarity
    ...
    References
    ----------
    Hill, Felix et al., "Simlex-999: Evaluating semantic models with (genuine) word_similarity estimation", 2014

    Notes
    -----
     SimLex-999 is a gold standard resource for the evaluation of models that learn the meaning of words and concepts.
     SimLex-999 provides a way of measuring how well models capture word_similarity, rather than relatedness or
